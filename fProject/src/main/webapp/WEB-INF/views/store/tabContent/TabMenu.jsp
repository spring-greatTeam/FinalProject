         ...
         <div id="menudiv">
          ...
          <c:if test="${group.groupNo eq menu.categoryNo}">
              <button style="font-size: 20px;"
                ...
                data-member-userid="${sessionScope.userId}"
...
<form action="${pageContext.request.contextPath}/cart/cart.me" method="post">
  ...
  <input type="hidden" id="opName" name="opName" value="없음" />
...
<script>
  $(document).on('click', '.btn-primary', function() {
       ...
       $.ajax({
         ...
         $("input[name='option']").on('change', updateTotalPrice);
       $("input[name='rbtn']").on('change', updateTotalPrice);
       ...
       var rbtn = "";
         // 체크박스 반복문 (체크된 것들 곱해서 수량 합을 totalPrice에 더하기)
         ...
         // 라디오 버튼이 선택된 경우 totalPrice에 추가하기
  ...
  $(document).on('click', '#plusbtn', function(){
        ...
        if ($("input[name='option']:checked").length > 0) {
         ...
         if ($("input[name='rbtn']:checked").length > 0) {
             var racheck = parseInt($("input[name='rbtn']:checked").val());
             totalPrice += racheck * quantity;
         }
         $("#totalPrice").val(totalPrice);
     });
